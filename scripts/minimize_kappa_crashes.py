#!/usr/bin/env python3
"""Minimize saved fuzz crashes and export recycled seed programs."""

from __future__ import annotations

import argparse
import json
import re
import subprocess
import tempfile
from dataclasses import dataclass
from pathlib import Path


def repo_root_from_script() -> Path:
    return Path(__file__).resolve().parents[1]


def first_nonblank_line(text: str) -> str:
    for line in text.splitlines():
        stripped = line.strip()

        if stripped:
            return stripped

    return ""


def classify_crash(returncode: int, stdout: str, stderr: str, timed_out: bool) -> str:
    if timed_out:
        return "timeout"

    lowered = f"{stdout}\n{stderr}".lower()

    if returncode < 0:
        return "crash"

    if "stack overflow" in lowered:
        return "crash"

    if "unhandled exception" in lowered:
        return "crash"

    if "stack trace" in lowered:
        return "crash"

    if "system." in stderr.lower():
        return "crash"

    if returncode == 0:
        return "ok"

    return "failure"


def run_compile(cli_path: Path, repo_root: Path, source: str, timeout_seconds: float) -> tuple[int, str, str, bool]:
    with tempfile.TemporaryDirectory(prefix="kappa-min-crash-") as temp_dir:
        temp_root = Path(temp_dir)
        source_path = temp_root / "main.kp"
        source_path.write_text(source.rstrip() + "\n", encoding="utf-8")

        try:
            result = subprocess.run(
                [str(cli_path), "--source-root", str(temp_root), str(source_path)],
                cwd=repo_root,
                capture_output=True,
                text=True,
                timeout=timeout_seconds,
            )
            return result.returncode, result.stdout, result.stderr, False
        except subprocess.TimeoutExpired as ex:
            return -1, ex.stdout or "", ex.stderr or "", True


def preserves_crash(cli_path: Path, repo_root: Path, source: str, expected_marker: str, timeout_seconds: float) -> bool:
    returncode, stdout, stderr, timed_out = run_compile(cli_path, repo_root, source, timeout_seconds)
    kind = classify_crash(returncode, stdout, stderr, timed_out)

    if kind != "crash":
        return False

    combined = f"{stdout}\n{stderr}"
    return expected_marker in combined if expected_marker else True


def split_blocks(text: str) -> list[str]:
    blocks: list[str] = []
    current: list[str] = []

    for line in text.splitlines():
        if line.strip() == "":
            if current:
                blocks.append("\n".join(current))
                current = []
        else:
            current.append(line)

    if current:
        blocks.append("\n".join(current))

    return blocks


def join_blocks(blocks: list[str]) -> str:
    return "\n\n".join(blocks).strip()


def ddmin_list(items: list[str], test) -> list[str]:
    if len(items) <= 1:
        return items

    n = 2

    while len(items) >= 2:
        chunk_size = max(1, len(items) // n)
        reduced = False

        for start in range(0, len(items), chunk_size):
            complement = items[:start] + items[start + chunk_size :]

            if complement and test(complement):
                items = complement
                n = max(2, n - 1)
                reduced = True
                break

        if not reduced:
            if n >= len(items):
                break

            n = min(len(items), n * 2)

    return items


def rewrite_candidates(source: str) -> list[str]:
    candidates: list[str] = []

    lines = [line for line in source.splitlines() if line.strip()]

    if not lines:
        return candidates

    without_module = [line for line in lines if not line.startswith("module ")]

    if without_module != lines:
        candidates.append("\n".join(without_module))

    type_line = next((line for line in lines if line.startswith("type ")), "")
    let_line = next((line for line in lines if line.startswith("let ")), "")

    type_match = re.match(r"type\s+([A-Za-z_][A-Za-z0-9_]*)\s*=\s*\((.*)\)", type_line)

    if type_match:
        type_name = type_match.group(1)
        candidates.append("\n".join([f"type {type_name} = (left : {type_name})"] + [line for line in lines if line != type_line]))

        if let_line:
            let_match = re.match(
                r"let\s+([A-Za-z_][A-Za-z0-9_]*)(?:\s*\([^)]*\))?\s*:\s*([A-Za-z_][A-Za-z0-9_]*)\s*=\s*(.*)",
                let_line,
            )

            if let_match:
                binding_name = let_match.group(1)
                result_type = let_match.group(2)
                rewritten = [f"type {type_name} = (left : {type_name})", f"let {binding_name} : {result_type} = ()"]
                candidates.append("\n".join(rewritten))

    if let_line:
        let_match = re.match(
            r"let\s+([A-Za-z_][A-Za-z0-9_]*)(?:\s*\([^)]*\))?\s*:\s*([A-Za-z_][A-Za-z0-9_]*)\s*=\s*(.*)",
            let_line,
        )

        if let_match:
            binding_name = let_match.group(1)
            result_type = let_match.group(2)
            simplified_let = f"let {binding_name} : {result_type} = ()"
            candidates.append("\n".join([simplified_let if line == let_line else line for line in lines]))

    unique_candidates: list[str] = []
    seen: set[str] = set()

    for candidate in candidates:
        normalized = candidate.strip()

        if normalized and normalized not in seen and normalized != source.strip():
            seen.add(normalized)
            unique_candidates.append(normalized)

    return unique_candidates


def minimize_crash_source(cli_path: Path, repo_root: Path, source: str, expected_marker: str, timeout_seconds: float) -> str:
    blocks = split_blocks(source)

    def block_test(candidate_blocks: list[str]) -> bool:
        candidate = join_blocks(candidate_blocks)
        return preserves_crash(cli_path, repo_root, candidate, expected_marker, timeout_seconds)

    if len(blocks) > 1:
        blocks = ddmin_list(blocks, block_test)

    source = join_blocks(blocks)
    lines = source.splitlines()

    def line_test(candidate_lines: list[str]) -> bool:
        candidate = "\n".join(candidate_lines).strip()
        return preserves_crash(cli_path, repo_root, candidate, expected_marker, timeout_seconds)

    if len(lines) > 1:
        lines = ddmin_list(lines, line_test)

    source = "\n".join(lines).strip()

    changed = True

    while changed:
        changed = False

        for candidate in rewrite_candidates(source):
            if len(candidate) >= len(source):
                continue

            if preserves_crash(cli_path, repo_root, candidate, expected_marker, timeout_seconds):
                source = candidate
                changed = True
                break

    return source


@dataclass
class CaseResult:
    name: str
    crash_marker: str
    original_lines: int
    minimized_lines: int
    changed: bool


def main() -> None:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--in-dir", default="artifacts/fuzzball-kappa-recycled/fuzz-run-freshbuild/crashes")
    parser.add_argument("--out-dir", default="artifacts/fuzzball-kappa-recycled/minimized-crashes")
    parser.add_argument("--seed-dir", default="artifacts/fuzzball-kappa/recycled-seeds")
    parser.add_argument("--cli", default="src/Kappa.Compiler.Cli/bin/Debug/net10.0/Kappa.Compiler.Cli")
    parser.add_argument("--timeout-seconds", type=float, default=5.0)
    args = parser.parse_args()

    repo_root = repo_root_from_script()
    in_dir = Path(args.in_dir)
    out_dir = Path(args.out_dir)
    seed_dir = Path(args.seed_dir)
    cli_path = Path(args.cli)

    if not in_dir.is_absolute():
        in_dir = repo_root / in_dir

    if not out_dir.is_absolute():
        out_dir = repo_root / out_dir

    if not seed_dir.is_absolute():
        seed_dir = repo_root / seed_dir

    if not cli_path.is_absolute():
        cli_path = repo_root / cli_path

    out_dir.mkdir(parents=True, exist_ok=True)
    seed_dir.mkdir(parents=True, exist_ok=True)

    results: list[CaseResult] = []

    for case_dir in sorted(path for path in in_dir.iterdir() if path.is_dir()):
        source = (case_dir / "main.kp").read_text(encoding="utf-8")
        stderr = (case_dir / "stderr.txt").read_text(encoding="utf-8")
        stdout = (case_dir / "stdout.txt").read_text(encoding="utf-8")
        crash_marker = first_nonblank_line(stderr) or first_nonblank_line(stdout)

        minimized = minimize_crash_source(
            cli_path,
            repo_root,
            source,
            crash_marker,
            args.timeout_seconds,
        )

        out_case_dir = out_dir / case_dir.name
        out_case_dir.mkdir(parents=True, exist_ok=True)
        (out_case_dir / "main.kp").write_text(minimized + "\n", encoding="utf-8")
        (out_case_dir / "expected.txt").write_text(crash_marker + "\n", encoding="utf-8")

        seed_case_dir = seed_dir / "crashes" / case_dir.name
        seed_case_dir.mkdir(parents=True, exist_ok=True)
        (seed_case_dir / "main.kp").write_text(minimized + "\n", encoding="utf-8")

        results.append(
            CaseResult(
                name=case_dir.name,
                crash_marker=crash_marker,
                original_lines=len(source.splitlines()),
                minimized_lines=len(minimized.splitlines()),
                changed=minimized.strip() != source.strip(),
            )
        )

    summary = {
        "count": len(results),
        "changed": sum(1 for result in results if result.changed),
        "results": [result.__dict__ for result in results],
    }
    (out_dir / "summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
    print(json.dumps(summary, indent=2))


if __name__ == "__main__":
    main()
