...
- [ ] Create `DEPENDENT_PROOF_ISSUE_CANDIDATES.md` for the new Tier 1 proof / totality / erasure repos not already covered by `TOTALITY_ISSUE_CANDIDATES.md`.
...
- [x] Kotlin hash-based collection constraint note
...
| Wave 10: Runtime, fibers, interruption, and resource cleanup | `first_candidate_note_started` | `RUNTIME_CONCURRENCY_ISSUE_CANDIDATES.md`; first manifest/keyword tranche covers Chapel, Pony, Encore, Elixir, and Inko inventory; direct reads include Chapel structured task synchronization / cleanup, Pony finalization / monitoring / stdio blocking, Encore future completion and tracing, Elixir supervisor/startup/interrupt/receive cases, and Inko process scheduling / receive waits / scoped panics / blocking pools / destructor escape | backfill PR/test artifacts for the accepted-later runtime candidates; read harvested runtime tests for Inko/Chapel/Pony/Encore/Elixir; stage runtime fixtures only after the local parser/runtime surface can handle the standard `fork`, `await`, `Scope`, `Monitor`, `timeout`, `race`, `defer`, and `using` shapes |
| Wave 11: Dependent proof, erasure, and elaboration | `planned_new_corpus_wave` | the corpus now has broad dependent/proof compiler coverage beyond the existing `lean4` / `Idris2` / `agda` notes: `FStar`, `rocq`, `Arend`, `cooltt`, `cubical`, `narya`, `rzk`, `smalltt`, `ATS-Postiats`, `ATS-Xanadu`, and `1lab` | create `DEPENDENT_PROOF_ISSUE_CANDIDATES.md`; first tranche should search `total`, `termination`, `decreases`, `erased`, `irrelevant`, `universe`, `level`, `transport`, `equality`, `coverage`, `impossible`, `with`, `elaboration`, `normalization`; avoid proof-library-only failures |
...
| `Idris2` | `TOTALITY_ISSUE_CANDIDATES.md` | full-corpus keyword scan for `termination`, `total`, `covering`, `with rule`, `with function` | exact body reads: `#19`, `#403`, `#493` | totality note complete; staged batch covers the direct recursive-cycle, alias, and branch-split analogues | continue with `#164`, `#300`, `#402`, `#430`, `#484` |
...
| `kotlin` | `KOTLIN_ISSUE_CANDIDATES.md` | full-corpus keyword scan for `?.`, `?:`, `safe call`, `elvis`, `smart cast`, `smartcast`, `flow typing`, `nullability`, `nullable`, `null check`; after the 2026-04-24 YouTrack layout refresh, issue ids must be read from `idReadable`, not internal `id` | exact body reads are enumerated in `KOTLIN_ISSUE_CANDIDATES.md`; latest incremental read attempt classified `KT-578`-`KT-581` and found `KT-582` absent; confirmed absent in current corpus: `KT-269`, `KT-271`, `KT-315`, `KT-356`, `KT-365`, `KT-373`, `KT-414`, `KT-420`, `KT-430`, `KT-452`, `KT-477`, `KT-488`, `KT-499`, `KT-506`, `KT-519`, `KT-522`, `KT-532`, `KT-539`, `KT-540`, `KT-541`, `KT-543`, `KT-544`, `KT-545`, `KT-546`, `KT-547`, `KT-548`, `KT-558`, `KT-561`, `KT-563`, `KT-564`, `KT-565`, `KT-567`, `KT-582` | Kotlin note complete through `KT-582`; `KT-549` and `KT-571` are staged as contextual lambda inference; `KT-566` is covered by the staged iterator-protocol fixture; `KT-570` is covered by real import-alias fixtures; `KT-577` is staged as a non-optional Elvis negative; `KT-550`, `KT-552`, `KT-556`, `KT-557`, `KT-572`, `KT-574`, `KT-575`, `KT-576`, `KT-578`, `KT-579`, `KT-580`, and `KT-581` are spec-addition ergonomics/diagnostic notes; `KT-551`, `KT-553`, `KT-554`, `KT-555`, `KT-559`, `KT-560`, `KT-562`, `KT-568`, `KT-569`, and `KT-573` are compiler-internal, backend, Kotlin-syntax, diagnostic, incomplete-code, or OO-inheritance background | continue with `KT-583`, `KT-584`, `KT-585`, `KT-586`, `KT-587` by `idReadable`; keep `KT-247` visible as already-mined safe-navigation coverage, `KT-2127` / `KT-8492` in the ergonomics/spec-follow-up bucket, `KT-2146` in the spec-clarity bucket, `KT-58` in the later finalization bucket, and the `!!` / mutable-local tranche in the intentional-divergence bucket |
...
| `chapel`, `ponyc`, `encore`, `elixir`, `inko` | corresponding per-repo manifests under `repos/`; Elixir issues are still referenced through legacy `../../elixir-issues.json` | `runtime_concurrency_resources` | `fiber`, `task`, `thread`, `actor`, `async`, `await`, `cancel`, `interrupt`, `timeout`, `race`, `scheduler`, `supervisor`, `scope`, `defer`, `finally`, `cleanup`, `resource`, `atomic`, `stm`; Encore pass also searched `future`, `active object`, `get`, `blocking`; Elixir pass also searched `gen_server`, `supervisor`, `assert_receive`; Inko pass also searched `process`, `recover`, `throw`, `panic`, `drop` | first runtime/concurrency note created: `RUNTIME_CONCURRENCY_ISSUE_CANDIDATES.md`; direct reads recorded for `chapel#5608`, `#5751`, `#6316`, `#6334`; `ponyc#34`, `#205`, `#224`, `#344`, `#350`; `encore#184`, `#274`, `#412`, `#429`, `#758`, `#802`, `#880`; `elixir#392`, `#849`, `#869`, `#982`, `#1566`, `#1724`; `inko#110`, `#112`, `#113`, `#213`, `#216`, `#254`, `#256`, `#278`, `#316`, `#321`, `#332`, `#339`; Inko harvested runtime test paths found but not read | backfill PR/test artifacts for accepted candidates; read Inko `test_async_await`, `test_process`, `test_drop`, `test_sync`, and linked runtime PRs next |
...
- Create `RUNTIME_CONCURRENCY_ISSUE_CANDIDATES.md` before staging tests.
