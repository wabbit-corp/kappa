...
- [ ] Replace the current source-text origin recovery in the resource checker with parser-carried spans before broadening the checker beyond root-place do-body forms.
...
- [x] Add `Releasable IO a` dictionary/evidence handling for the `using` KCore lowering path, reusing the existing M2 trait machinery where possible.
  The current slice generalizes lightweight trait constraints/instances to multiple arguments and records selected `Releasable m a` evidence in the KCore `Release` action; backend cleanup invocation is still tracked by the CLR/Zig lowering items below.
...
- [ ] Add checkpoint-level tests for QTT metadata once the resource checker starts attaching ownership, borrow-region, and place facts to KFrontIR/KCore observability.
...
Current M3 status note: started with the source/KFrontIR surface slice and the first enforceable BODY_RESOLVE ownership slice. The compiler now parses and preserves typed quantity metadata for function parameters and do-bind patterns, parses borrowed binders with optional explicit regions, parses `using pat <- expr` with the spec-required rejection of explicit quantity markers, and parses `inout` parameters plus `~` call-site markers. Data-driven fixtures cover those surfaces plus the required M3 resource-checker contract cases: dropped owned resource, sequential overuse, single-application overuse, borrowed-region escape, downward-only borrowed closure, and branch-join consumption. The first resource checker enforces linear drop/overuse and basic using-region closure escape for the supported root-place do-body subset, reports stable diagnostic codes at `KFrontIR.BODY_RESOLVE`, records consume/borrow/capture/escape ownership events, exposes initial ownership fact groups in KFrontIR JSON/S-expression dumps, reports primary plus related origins for the supported M3 diagnostics, and emits deferred ownership facts for unsupported loop/match analyses instead of omitting them. KCore and KRuntimeIR now preserve the first protected-resource shape with `DoScope`, `ScheduleExit`, and `Release`; `using` release actions can select multi-parameter `Releasable m a` evidence at KCore. Runtime evaluation and backend lowering still treat that scheduling as transparent, so CLR/Zig cleanup codegen remains open. The next M3 slice should lower release actions into concrete cleanup behavior. Parser-carried spans should replace the temporary source-text origin recovery before the supported checker subset grows much wider.
