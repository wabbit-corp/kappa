...
- [ ] Convert checkpoint verification and target-checkpoint diagnostics to typed evidence ADTs with centralized formatting.
  Target-checkpoint diagnostics are now typed. `CheckpointVerification.fs` now uses typed evidence for the surface-source / KFrontIR / KCore identity-and-phase contract failures (duplicate file identities, missing line tables, missing EOF tokens, resolved-phase mismatches, premature ownership facts, duplicate module identities). The remaining raw emitted checkpoint-verification diagnostics are the deeper KRuntimeIR / KBackendIR verifier families.
