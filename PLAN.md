...
- [ ] Finish the remaining Appendix-T gaps without overstating current conformance. Already implemented: `suite.ktest`, `incremental.ktest` parsing plus step-suite execution, `mode`, `packageMode`, `scriptMode`, `backend`, `entry`, `runArgs`, `stdinFile`, `dumpFormat`, standard `requires capability <name>`, `requires backend <profile>`, `requires mode package`, and `requires mode script`, `assertDiagnostic*` including source-anchored `assertDiagnosticNext`, deprecated `assertDiagnosticHere`, and exact-range `assertDiagnosticAt ... startLine startColumn - endLine endColumn`, `assertType` by definitional equality, `assertFileDeclKinds`, `assertStageDump`, `assertStderrFile`, trace-count assertions, and the cross-step assertions `assertStepNoErrors`, `assertStepErrorCount`, `assertStepWarningCount`, and `assertStepTraceCount`.
- [ ] Keep pushing primary-origin coverage deeper into compiler diagnostics so Appendix-T line/range assertions keep becoming more meaningful instead of relying on broad fallback attachment at binding-validation boundaries. This slice now backfills missing definition-body origins for top-level binding diagnostics during validation, which is enough for the current fixture corpus but still not the end-state.
...
- [x] Convert backend/lowering diagnostics to typed evidence ADTs with centralized formatting.
