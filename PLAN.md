...
- [ ] Convert checkpoint verification and target-checkpoint diagnostics to typed evidence ADTs with centralized formatting.
  Target-checkpoint diagnostics are now typed. `CheckpointVerification.fs` now uses typed evidence for the surface-source / KFrontIR / KCore identity-and-phase contract failures, plus the KRuntimeIR module/binding structural checks (missing imported runtime modules, duplicate runtime bindings/constructors, unsupported intrinsic terms, malformed intrinsic bindings, missing runtime binding bodies, duplicate runtime pattern/closure binders, and undisugared deep handlers). The remaining raw emitted checkpoint-verification diagnostics are the deeper KRuntimeIR type/constructor/body invariants and the KBackendIR verifier families.
