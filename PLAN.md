...
- [ ] Convert checkpoint verification and target-checkpoint diagnostics to typed evidence ADTs with centralized formatting.
  Target-checkpoint diagnostics are now typed. `CheckpointVerification.fs` now uses typed evidence for the surface-source / KFrontIR / KCore identity-and-phase contract failures, the KRuntimeIR module/binding structural checks (missing imported runtime modules, duplicate runtime bindings/constructors, unsupported intrinsic terms, malformed intrinsic bindings, missing runtime binding bodies, duplicate runtime pattern/closure binders, and undisugared deep handlers), and the KBackendIR module/function/erasure/pattern/expression checks (missing backend modules/imports, duplicate functions/data layouts/environment layouts, unsupported backend intrinsic terms, entry-point consistency, calling-convention consistency, environment-layout references, intrinsic-function shape, missing function bodies, pre-erasure runtime metadata leaks, backend pattern/or-pattern constructor validation, unresolved runtime names, closure shape/layout invariants, call convention invariants, and constructed-data layout validation). The remaining raw emitted checkpoint-verification diagnostics are the residual unknown-checkpoint/simple fallback and any future verifier families added outside these typed paths.
