...
- [ ] Finish the remaining `SurfaceElaboration.fs` diagnostics conversion by removing the generic `makeDiagnostic` / `makeCodeDiagnostic` escape hatches that still assemble raw prose inline.
  `ResourceChecking.fs` no longer emits raw `DiagnosticFact.simple`, `DiagnosticFact.detail`, or `DiagnosticFact.codeDetail` diagnostics. In `SurfaceElaboration.fs`, the record/row/record-patch validation cluster and the trailing record-type duplicate/cycle validation path now use typed `SurfaceRecordDiagnosticEvidence`; the static-object / active-pattern / projection-definition / top-level module-and-binding / malformed-constructor / seal-and-record-projection / projector-descriptor validation clusters now use typed `SurfaceElaborationDiagnosticEvidence`; the generic expected/actual mismatch path now emits structured `ExpectedActualTypeMismatch` payloads instead of raw prose; the member-call / application-shape / non-callable-application / member-access receiver / safe-navigation / elvis / implicit-application / named-application call-site diagnostics now route through typed `SurfaceElaborationDiagnosticEvidence`; the recurring trait-constraint unresolved / ambiguous / missing-supertrait diagnostics now route through typed `SurfaceElaborationDiagnosticEvidence`; contextual unresolved-name messaging now goes through structured `NameUnresolvedEvidence` instead of inline prose; the syntax-carrier escape, prefixed-string prefix, projected-field mismatch, constructor-vs-`Type`, numeric-literal range overflow, `failElabWith` surface wrappers, query-carrier/comprehension diagnostics, refutable generator / left-join lexical validation, arithmetic diagnostics, short-circuit diagnostics, object-phase-to-elaboration diagnostics, quote/top-level splice carrier diagnostics, non-bindable `<-` carrier diagnostics, handler validation diagnostics, constructor-default validation diagnostics, kind-qualified static-object unresolved diagnostics, borrowed-resumption / list-cons / macro-linearity / or-pattern / instance-member-return / exported-multishot backend diagnostics, and the remaining `CORE_LOWERING` mismatch helpers now also route through typed evidence. The main remaining `SurfaceElaboration` raw cluster is the location-bearing helper path, especially `makeLocatedDiagnostic` and the generic wrappers still built around it.
- [ ] Convert the remaining `ElaborationEvaluation.fs` diagnostics to typed evidence ADTs with centralized formatting.
  The macro builtin paths in `ElaborationEvaluation.fs` (`failElab`, `warnElab`, `failElabWith`, `warnElabWith`) now route through typed macro-expansion evidence, but the generic macro-expansion simple-diagnostic helper path still remains.
- [ ] Convert any remaining typechecking/validation diagnostics outside `SurfaceElaboration.fs` and `ElaborationEvaluation.fs` to typed evidence ADTs with centralized formatting.
  The stray `~` marker / borrowed-implicit-escape checks already route through the dedicated QTT evidence ADTs.
