...
- [ ] Convert checkpoint verification and target-checkpoint diagnostics to typed evidence ADTs with centralized formatting.
  Target-checkpoint diagnostics are now typed. `CheckpointVerification.fs` now uses typed evidence for the surface-source / KFrontIR / KCore identity-and-phase contract failures, the KRuntimeIR module/binding structural checks (missing imported runtime modules, duplicate runtime bindings/constructors, unsupported intrinsic terms, malformed intrinsic bindings, missing runtime binding bodies, duplicate runtime pattern/closure binders, and undisugared deep handlers), and the KBackendIR module/function/erasure checks (missing backend modules/imports, duplicate functions/data layouts/environment layouts, unsupported backend intrinsic terms, entry-point consistency, calling-convention consistency, environment-layout references, intrinsic-function shape, missing function bodies, and pre-erasure runtime metadata leaks). The remaining raw emitted checkpoint-verification diagnostics are the KBackendIR expression/closure/pattern verifier families plus any residual unknown-checkpoint/simple fallback.
