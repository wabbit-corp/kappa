...
- [ ] Lower `using` exit actions in the CLR backend to `try` / `finally`, calling the selected `Releasable` dictionary's release implementation exactly once.
  First normal-exit cleanup invocation is implemented and preserves the protected body result; full CLR unwinding through real `try` / `finally` remains open.
