...
- [ ] Convert backend/lowering diagnostics to typed evidence ADTs with centralized formatting.
  ...
  The remaining backend work is the still-raw plain CLR resolution/emission failures (`IlDotNetBackendInput.fs`, `IlDotNetBackendTyping.fs`, `IlDotNetBackendEmit.fs`), the outer CLR artifact-emission wrapper failures, and the Zig/backend support stack.
