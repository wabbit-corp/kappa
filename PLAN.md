...
- [x] Finish `CoreParsing.fs` diagnostics conversion for the remaining projection and selector structural paths.
  Qualified-name recovery, selector-name recovery, and projection accessor-clause recovery now route through `CoreExpressionParsingEvidence`, alongside the earlier named application, record-literal field, and record-patch item validation diagnostics.
- [x] Finish `CoreParsing.fs` diagnostics conversion for the remaining primary-expression recovery paths.
  Local `let ... in ...` recovery, missing `)` recovery, the generic expected-expression path, interpolated-string recovery, `if`-expression `then` / `else`, unmatched enclosure-close diagnostics, pure-block empty/final-expression diagnostics, generic expected-keyword recovery, and qualified-name recovery now route through `CoreExpressionParsingEvidence`.
- [x] Finish `CoreParsing.fs` diagnostics conversion for projection headers, accessor clauses, and projection-specific binder validation.
  Projection-body diagnostics, accessor-clause recovery, reachable `set`-parameter validation, place-binder recovery, illegal `inout`, missing result-type `:`, unterminated projection binders, unsupported projection-binder syntax, and malformed `set` parameter-shape recovery now all route through structured evidence.
- [x] Finish `CoreParsing.fs` diagnostics conversion for remaining handler/projection validation diagnostics.
  Handler-expression syntax, return-clause validation, query/comprehension diagnostics, function/local-function header diagnostics, and expression-tail diagnostics now all route through `CoreExpressionParsingEvidence`.
...
- [x] Finish the remaining `SurfaceElaboration.fs` diagnostics conversion by removing the generic `makeDiagnostic` / `makeCodeDiagnostic` escape hatches that still assemble raw prose inline.
  `ResourceChecking.fs` no longer emits raw `DiagnosticFact.simple`, `DiagnosticFact.detail`, or `DiagnosticFact.codeDetail` diagnostics. `SurfaceElaboration.fs` now likewise routes its record/row/record-patch validation, static-object / active-pattern / projection-definition / top-level module-and-binding / malformed-constructor / seal-and-record-projection / projector-descriptor validation, generic expected/actual mismatch, member-call / application-shape / non-callable-application / member-access receiver / safe-navigation / elvis / implicit-application / named-application call-site, trait-constraint unresolved / ambiguous / missing-supertrait, contextual unresolved-name, syntax-carrier / prefixed-string / projected-field / constructor-vs-`Type` / numeric-literal range / `failElabWith`, query / arithmetic / short-circuit / object-phase / splice-carrier / non-bindable `<-`, handler / constructor-default / kind-qualified static-object unresolved, borrowed-resumption / list-cons / macro-linearity / or-pattern / instance-member-return / exported-multishot backend, and location-bearing `inout` diagnostics through typed ADTs with centralized formatting. `SurfaceElaboration.fs` no longer emits raw `DiagnosticFact.simple`, `DiagnosticFact.detail`, or `DiagnosticFact.codeDetail` diagnostics.
- [x] Convert the remaining `ElaborationEvaluation.fs` diagnostics to typed evidence ADTs with centralized formatting.
  `ElaborationEvaluation.fs` now routes macro builtin paths (`failElab`, `warnElab`, `failElabWith`, `warnElabWith`) and syntax-value capture-escape failures through typed macro-expansion evidence. It no longer emits raw `DiagnosticFact.simple`, `DiagnosticFact.detail`, or `DiagnosticFact.codeDetail` diagnostics.
- [x] Convert any remaining typechecking/validation diagnostics outside `SurfaceElaboration.fs` and `ElaborationEvaluation.fs` to typed evidence ADTs with centralized formatting.
  `ResourceChecking.fs`, the stray `~` marker / borrowed-implicit-escape checks, and the macro-expansion escape path now all route through dedicated typed evidence ADTs. No remaining emitted `DiagnosticFact.simple`, `DiagnosticFact.detail`, or `DiagnosticFact.codeDetail` sites remain in ordinary typechecking/validation outside checkpoint verification.
- [ ] Convert backend/lowering diagnostics to typed evidence ADTs with centralized formatting.
  `Compilation.fs` backend-runtime capability checks, `CompilationCheckpoints.fs` target-checkpoint diagnostics, and `KBackendLowering.fs` lowering failures now route through typed backend/target-checkpoint evidence. The remaining backend work is deeper emitter/support layers that still return raw string failures, especially the Zig backend emit/artifact/support stack.
  The plain CLR emitter/typing lane now routes its common unsupported-shape failures through typed `ClrBackendEmitterErrorEvidence` centralized in `Diagnostics.fs` instead of hardcoding prose inline (`runtime closures`, prefixed strings, effect handlers in the non-effectful lane, function-/constructor-valued names, unary operators, unsupported builtin operator overloads, unsupported intrinsic overloads, and recursive type inference gaps).
  Plain CLR or-pattern agreement, constructor-pattern resolution/arity, constructor arity, and missing-body failures now also route through `ClrBackendEmitterErrorEvidence` in both `IlDotNetBackendTyping.fs` and `IlDotNetBackendEmit.fs`.
  ...
  Plain CLR binding/declaration-shape failures in `IlDotNetBackendTyping.fs` now also route through `ClrBackendEmitterErrorEvidence`, including bodyless bindings without return types, parameterized bindings without declared parameter types, bodyless parameterized bindings without explicit return types, expression-type mismatches, literal-pattern mismatches, trait-call result ambiguity, and unresolved `is` constructors.
  `IlDotNetBackendInput.fs` now also routes declared-type resolution failures plus binding/constructor call-arity failures through `ClrBackendEmitterErrorEvidence`.
  ...
  The remaining backend work is the still-raw plain CLR lowering/emission failures in `IlDotNetBackendInput.fs` (notably `Dict`-shape validation, unsupported type-form lowering, unification failures, and generic type-argument inference) and `IlDotNetBackendEmit.fs`, plus the Zig/backend support stack.
...
- `IntrinsicCatalog` now exposes a typed intrinsic-spec lookup, derives bundled-prelude \`expect term\` runtime arity structurally from authoritative parsed declarations, no longer treats unknown intrinsic names as implicitly nullary, and no longer classifies ordinary prelude definitions like `not` / `and` / `or` or constructors like `True` / `False` as intrinsic specs. Operator classification, builtin prelude trait-helper synthesis, and canonical builtin head-type matching now live in `KnownPreludeSemantics.fs` instead of `IntrinsicCatalog`. It is still not authoritative across the compiler: arity/result/availability/backends are still duplicated outside that catalog.
...
- [ ] Remove the stale `Constraint` / `Dict` surface model and align trait evidence with §§5.1.3 and 7.3.
  The spec now says trait evidence is just a value of the trait evidence type itself: there is no separate `Constraint` classifier and no `Dict (Eq Int)` wrapper. Current frontend/backends/tests still contain `Dict`-shaped parsing, lowering, fixtures, and macro helpers that must be replaced with direct trait-evidence types plus ordinary implicit/explicit evidence passing.
