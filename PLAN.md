...
- [ ] Convert backend/lowering diagnostics to typed evidence ADTs with centralized formatting.
  ...
  The plain CLR emitter/typing lane now routes its common unsupported-shape failures through typed `ClrBackendEmitterErrorEvidence` centralized in `Diagnostics.fs` instead of hardcoding prose inline (`runtime closures`, prefixed strings, effect handlers in the non-effectful lane, function-/constructor-valued names, unary operators, unsupported builtin operator overloads, unsupported intrinsic overloads, and recursive type inference gaps). The remaining backend work is the effectful CLR emitter path plus the Zig/backend support stack.
