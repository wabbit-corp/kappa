...
- [ ] Convert the remaining elaboration/typechecking diagnostics to typed evidence ADTs with centralized formatting.
  `ResourceChecking.fs` no longer emits raw `DiagnosticFact.simple`, `DiagnosticFact.detail`, or `DiagnosticFact.codeDetail` diagnostics, and the record/row/record-patch validation cluster in `SurfaceElaboration.fs` now uses typed `SurfaceRecordDiagnosticEvidence`. The main remaining checker debt is the broader `SurfaceElaboration.fs` `SimpleDiagnosticKind` message sites and the `CORE_LOWERING` simple-diagnostic wrappers outside that record cluster.
