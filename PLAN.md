...
- [ ] Convert backend/lowering diagnostics to typed evidence ADTs with centralized formatting.
  ...
  The remaining backend work is the still-raw plain CLR lowering/emission failures in `IlDotNetBackendInput.fs` and `IlDotNetBackendEmit.fs`, plus the Zig/backend support stack.
