...
- [ ] Finish `CoreParsing.fs` diagnostics conversion for the remaining projection and selector structural paths.
  Named application, record-literal field, and record-patch item validation diagnostics now route through `CoreExpressionParsingEvidence`.
- [ ] Finish `CoreParsing.fs` diagnostics conversion for the remaining primary-expression recovery paths.
  Local `let ... in ...` recovery, missing `)` recovery, the generic �expected expression� path, interpolated-string recovery, `if`-expression `then` / `else`, unmatched enclosure-close diagnostics, and pure-block empty/final-expression diagnostics now route through `CoreExpressionParsingEvidence`, but a few selector/name recovery helpers still use raw detail strings.
- [ ] Finish `CoreParsing.fs` diagnostics conversion for projection headers, accessor clauses, and projection-specific binder validation.
  Projection-body diagnostics now route through `CoreExpressionParsingEvidence`, including malformed body heads, missing projection-match case structure, and missing `then` / `else` in projection `if` bodies. Projection accessor-arrow recovery, reachable `set`-parameter validation diagnostics, projection place-binder recovery, illegal `inout`, missing result-type `:`, unterminated projection binders, unsupported projection-binder syntax, and malformed `set` parameter-shape recovery now also route through structured evidence. Some generic accessor-clause/header fallback paths still use raw detail strings.
