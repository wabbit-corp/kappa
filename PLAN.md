...
- [x] Add the first ownership event model for the supported subset: consume, borrow, capture, escape, and simple linear move events are recorded and serialized from BODY_RESOLVE. Copy-forbidden events are still part of the full resource-context work above.
...
- [ ] Implement syntax-directed context splitting and consumption for applications, `let`, `do`, `match`, lambdas/closures, and control-flow joins.
  Simple do-let linear moves are now handled for root places: `let alias = file` consumes the original linear binding and transfers the linear drop obligation to the alias, preventing accidental duplication while allowing rename-style moves.
- [ ] Implement the quantity satisfaction relation from `Spec.md`, including the strict separation between owned (`1`) and borrowed (`&`) obligations.
  `ResourceQuantity.satisfies` now implements the spec table for intervals and borrow mode; the checker uses the same internal model for resource signatures and bindings. First strict-separation rules are covered: passing a borrowed `using` binding or a local `let &`/bind-pattern borrow to a consuming `1` parameter now reports `E_QTT_BORROW_CONSUME`.
...
- [x] Lower `using` exit actions in the ZigCc backend with equivalent cleanup behavior, even if the first implementation uses a small generated-C runtime shim.
  ZigCc now emits panic frames around scheduled-exit result bindings so cleanup runs on normal completion and while propagating backend panics such as non-exhaustive matches.
