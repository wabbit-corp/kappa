...
- [x] Introduce a resource model with explicit invariants: interval quantities are separate from borrow mode; place is root plus path even when the first implementation only constructs root paths; borrow regions distinguish explicit user regions from anonymous rigid regions and carry owner scope; `using` creates one region per protected scope, not per bound name.
  `ResourceModel.fs` now owns places, borrow regions, bindings, scope frames, and `using` obligations. `ResourceQuantity` separates interval capabilities from borrow mode, and `using` facts/regressions now prove that multi-binder protected scopes share one rigid region and one hidden release obligation.
...
- [x] Replace the simple name-to-type typing environment in the affected checker paths with a lexical resource context that tracks binder kind, origin, declared quantity, inferred demand or borrow obligation, current place, region environment, availability/consumption state, and active `using` obligations.
- [x] Implement syntax-directed context splitting and consumption for applications, `let`, `do`, `match`, lambdas/closures, and control-flow joins.
  The checker now uses `ResourceContext` through expression/do checking, branch joins, closure capture, alias transfer, match-pattern binding, and application demand checking instead of the earlier ad hoc name-only environment.
- [x] Implement the quantity satisfaction relation from `Spec.md`, including the strict separation between owned (`1`) and borrowed (`&`) obligations.
  `ResourceQuantity.satisfies` implements the interval/borrow table, overuse checking compares exact accumulated demand against capability, positive lower-bound drop checking is enforced, and borrowed-to-consuming mismatches now diagnose as `E_QTT_BORROW_CONSUME`.
- [x] Implement stable-place analysis and borrow introduction for variables, parameters, hidden temporaries, and the initial field/path subset required by M3.
- [x] Implement skolem borrow regions for `using` and borrowed binders, including closure-capture tainting and non-escape diagnostics.
  Borrow introduction now covers variables, parameters, hidden temporaries, record paths, constructor-field paths, selector-form projection calls, and projector-descriptor applications in the supported subset. Borrowed locals and parameters carry rigid region environments, and closure/syntax/value escape checks reject leaked anonymous regions.
- [x] Implement deterministic release scheduling for `using pat <- expr` as the primitive split from the spec: keep one hidden owned resource in the scope frame, expose only borrowed pattern bindings in the protected body, create one shared rigid region for the whole pattern, and schedule exactly-once release on all exits.
...
- [x] Implement `inout` / `~` as a type-directed elaboration rewrite after the resource context exists, not as parser-only sugar.
  Resolved `inout` parameters require `~`, `~` on ordinary parameters reports `E_QTT_INOUT_MARKER_UNEXPECTED`, and stable-place write-back now handles whole roots, stable subplaces, `var` roots, pure/monadic residuals, selector-form projection calls, projector descriptor applications, accessor-bundle openers, and multi-root record packs.
- [x] Define and implement a canonical ownership-fact dump schema at `KFrontIR.BODY_RESOLVE` and `KFrontIR.CHECKERS`.
...
- [x] Erase quantities, regions, and borrow-only metadata before `KBackendIR`, while preserving ordinary runtime values and retained backend generics/dictionaries.
- [x] Strengthen checkpoint verification so post-erasure IR rejects leaked quantity, borrow, region, or place-only constructs.
...
- [x] Stop for a cleanup pass after the first green M3 slice: separate resource-context logic, borrow-region logic, and backend cleanup lowering so M4 effect-handler work does not inherit a monolith.
...
- [x] Extend checkpoint verification and observability so post-erasure stages can prove that effect rows are gone while handler/resumption legality is still checked at the last effect-aware checkpoint.

Current M4 status note: started, not complete. The compiler now has a real effect-handler kernel in `KCore` / `KRuntimeIR`, one-shot scoped-effect execution in the interpreter, parser support for multiline handler clause bodies, imported/top-level effect-label resolution across module boundaries, an ownership-checked multi-shot capture rejection for live linear or borrowed continuation suffixes, and checkpoint coverage that rejects surviving handler runtime constructs or leaked `Eff`/effect-row metadata before `KBackendIR`. The remaining work is the full row-polymorphic type/effect solver, more precise clause typing over remainder rows, and non-interpreter backend support.
