...
- [ ] Extend `TypeSignatures.TraitConstraint` beyond the new parsed `TraitReference` shape so resolved declaration identity also survives through elaboration, instance search, dictionary synthesis, and backend lowering. Qualification is now preserved structurally, but later phases can still fall back to local trait names when they should be carrying semantic identity directly.
