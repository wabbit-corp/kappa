...
| 8.2 Do blocks | `spec_8_2_*`, M1/M2 milestone tests | compiler divergence | Basic sequencing, bind result, `printInt`, M2 stateful do paths, and the first M3 protected-scope `using` slice are covered; the full abrupt-completion / labeled-scope / `defer` model of §8.7 is still incomplete. |
