...
- [ ] Convert checkpoint verification and target-checkpoint diagnostics to typed evidence ADTs with centralized formatting.
  Target-checkpoint diagnostics are now typed. The remaining emitted raw-diagnostic site is `CheckpointVerification.fs`, which still constructs `DiagnosticFact.simple SimpleDiagnosticKind.CheckpointVerification ...` across its verification families.
