...
- [ ] Convert the remaining elaboration/typechecking diagnostics to typed evidence ADTs with centralized formatting.
  `ResourceChecking.fs` no longer emits raw `DiagnosticFact.simple`, `DiagnosticFact.detail`, or `DiagnosticFact.codeDetail` diagnostics. In `SurfaceElaboration.fs`, the record/row/record-patch validation cluster now uses typed `SurfaceRecordDiagnosticEvidence`, and the static-object / active-pattern / projection-definition validation clusters now use typed `SurfaceElaborationDiagnosticEvidence`. The main remaining checker debt is the broader `SurfaceElaboration.fs` `SimpleDiagnosticKind` message sites and the `CORE_LOWERING` simple-diagnostic wrappers outside those clusters.
