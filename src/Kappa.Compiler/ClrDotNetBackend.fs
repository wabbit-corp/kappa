...
module internal ClrDotNetBackend =
    ...
    let private lowerBinding (existingBinding: KRuntimeBinding option) (backendFunction: KBackendFunction) =
    ...
    let private mergeRuntimeModule
        (shellModules: Map<string, KRuntimeModule>)
    ...
    let private synthesizeRuntimeModules (workspace: WorkspaceCompilation) =
        let shellModules =
            workspace.KCore
        ...
        |> List.map (mergeRuntimeModule shellModules)
    ...
    let emitAssemblyArtifact (workspace: WorkspaceCompilation) (outputDirectory: string) =
        if workspace.HasErrors then
            ...
            if not (List.isEmpty verificationDiagnostics) then
                ...
                let runtimeModules = synthesizeRuntimeModules workspace
                IlDotNetBackend.emitAssemblyArtifact { workspace with KRuntimeIR = runtimeModules } outputDirectory
